...
And similar to rebasing, there exists also [Cherry-picking](#cherry-picking), which follows the same idea and is a good tool for more picky maintainers.

### When to use it 
Rebasing, in comparison with the end product, is not that different from the basic merge command. One of the biggest pros of using rebasing is, as it was mentioned before, that it makes for a cleaner commit history by rewriting it to linear state even though the work was previously parallel. Another benefit coming from using the ```git rebase``` command is that it limits the commits that would be needed to merge the same changes, so again, it leaves the commit history much more clean. ([Chacon & Straub, 2014](#pro-git-book)). <br>

### When not to use it 
The advantage of a cleaner tree that rebasing provides could be also the reason why not to use it. While rebasing would be usefull to make the commit history more readable, if the need for the outcome is that the tree should be more presentable or easy to orient in, still there would be the loss of a real work path that was taken to get to the final sate of the work.  With rebasing, problems might also occur when trying to rebase commits outside of the personal repository. Rebasing essentialy creates new slightly different commits which then pushed into the common repository can cause differences between coworkers' commit history and shared repository commits history, leaving them with only the option of re-merging their work. ([Chacon & Straub, 2014](#pro-git-book)). <br>

## Cherry-Picking
### General idea
Both ```git rebase``` and ```git cherry-pick``` are  commands used to rewrite the commits of one branch on top of another ([Geeks for Geeks: Git cherry-pick](#geeks)). Cherry-picking includes manually picking commits from one branch and introducing them to another branch ([Bunyakiati & Phipathananunth, 2017](#cherry_picking_code_commits)). This feature is particularly usefull if we have a number of commits on the topic branch and we want to integrate only one of them.  We use the ```git cherry-pick``` command to take the changes introduced in a single Git commit and re-introduce it as a new commit on the branch we are currently on ([Chacon & Straub, 2014](#pro-git-book)).
...
### When to use it
Cherry-picking requires thoughtful application. There are many possible uses for this command such as for example work in team-environments where precise coordination is essential. Cherry-picking enables workers to select specific commits and incorporate only the necessary changes. For instance, this practice can be usefull when rectifying accidental changes made to a wrong branch ([Git-cherry-pick](#geeks)).
...
### When to not use it
As appealing as it may seem, there are certain limitations to ```git cherry-pick``` command. Cherry-picking essentially creates two copies of the same commit. If one of the branches with the commit gets modified, it can potentially create challenges when attempting to merge these two branches in the future.  This issue might cause problems, especially while working on collaborative team projects, thus the advantage of this command can be considered a drawback ([Chen, 2018](#devblogs)).
